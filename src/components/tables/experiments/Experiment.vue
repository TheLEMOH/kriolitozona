...
const filters = ref<DataTableFilterMeta>({
  ...
  isPermafrost: { value: null, matchMode: FilterMatchMode.CONTAINS },
  permafrostBoundary: { value: null, matchMode: FilterMatchMode.CONTAINS },
  granularComposition: { value: null, matchMode: FilterMatchMode.CONTAINS },
...
<template>
  <Table :items="items" :filters="filters" @updateFilters="UpdateFilters" :filterDisplay="'row'" :globalFilterFields="global" :tableLayout="'fixed'">
    <template #columns>
      <Column field="name" header="Имя п/п" :showFilterMenu="false" style="width: 200px">
        <template #filter="{ filterModel, filterCallback }">
          <InputText v-model="filterModel.value" @input="filterCallback()" type="text" class="p-column-filter" placeholder="Поиск по имени" />
        </template>
      </Column>
      ...
      <Column field="soilProfileCondition" header="Состояние п/п" :showFilterMenu="false" style="width: 200px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="stateSoil" optionValue="label" optionLabel="label" placeholder="Выберите из списка">
          </Dropdown>
        </template>
      </Column>
      <Column field="landscape" header="Ландшафт" :showFilterMenu="false" style="width: 180px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="landscapes" optionValue="label" optionLabel="label" placeholder="Выберите из списка">
          </Dropdown>
        </template>
      </Column>
      <Column field="microrelief" header="Микрорельеф" :showFilterMenu="false" style="width: 180px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="microrelief" optionValue="label" optionLabel="label" placeholder="Выберите из списка">
          </Dropdown>
        </template>
      </Column>
      <Column field="typeOfSoil" header="Тип почвы" :showFilterMenu="false" style="width: 180px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="typesOfSoil" optionValue="label" optionLabel="label" placeholder="Выберите из списка">
          </Dropdown>
        </template>
      </Column>
      <Column field="isPermafrost" header="Присутствие мерзлоты" :showFilterMenu="false" style="width: 150px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="isExist" optionValue="label" optionLabel="label" placeholder="Выберите из списка"> </Dropdown>
        </template>
      </Column>
      <Column field="permafrostBoundary" header="Граница залегания мерзлоты" :showFilterMenu="false" style="width: 180px">
        <template #filter="{ filterModel, filterCallback }">
          <InputText v-model="filterModel.value" @input="filterCallback()" type="text" class="p-column-filter" placeholder="Поиск по имени" />
        </template>
      </Column>
      <Column field="granularComposition" header="Грансостав" :showFilterMenu="false" style="width: 120px">
        <template #filter="{ filterModel, filterCallback }">
          <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="compositions" optionValue="label" optionLabel="label" placeholder="Выберите из списка">
          </Dropdown>
        </template>
      </Column>
